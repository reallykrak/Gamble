...
async def start_cmd(message: Message):
    ...
    get_user(user.id) # Kullanıcıyı varsa yükle, yoksa oluştur
    await message.answer(f"👋 Merhaba <b>{user.first_name}</b>!\n🎰 Kumar Botuna Hoş Geldin!\n\n💵 Başlangıç bakiyen: <b>{DEFAULT_DATA['users'][str(user.id)]['bakiye']}₺</b>", parse_mode="HTML")
...
async def komutlar_cmd(message: Message):
    komutlar = (
        "🎮 /start – Botu başlat\n"
        "💰 /bakiye – Bakiye, banka ve döviz\n"
        "🎁 /bonus – Günlük bonus al\n"
        "🏦 /bankaparaekle <miktar> – Bakiyeden bankaya yatır\n"
        "💳 /bankaparaçek <miktar> – Bankadan bakiyeye çek\n"
        "💱 /banka – Banka bilgileri ve döviz kurları\n"
        "📈 /dövizal <tür> <miktar> – Bankadan döviz al (dolar, euro, sterlin, elmas)\n"
        "📉 /dövizsat <tür> <miktar> – Döviz satıp bankaya ekle\n"
        "🎰 /slot <miktar> – Slot çevir (3 aynı emoji x4 kazandırır)\n"
        "🔥 /risk <miktar> – %50 şansla bahsin 2 katını kazan\n"
        "⚽ /bahis <miktar> – Takım seçip bahis yap\n"
        "🤝 /paragönder <id> <miktar> – Başka kullanıcıya para gönder\n"
        "🆔 /id – Kullanıcı ID'sini göster (yanıtlarsan o kişinin)\n"
        "🏆 /top – En zenginler listesi (Bakiye + Banka)\n"
        "📜 /komutlar – Tüm komutlar listesi\n"
        "🛡 /admin <id> – Admin ekle (Sadece Admin)\n" # Admin komutu olduğunu belirt
        "🔍 /rep – Kullanıcı bilgisi (yanıtlarsan o kişinin)"
    )
